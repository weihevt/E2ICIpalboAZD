function E2ICIpalboAZD_genparcohort()
% This function generates opmat.mat, the first step is to create the
% parameter cohort.
expand = false;
...
if ~expand
    ...
    while 1 > 0
        ...
        E2ICIpalboAZD_findcohort(x0,args,lbuse,ubuse,args.groupparall,limit_par,opt,InitialPMuse,...
...
if expand
    ...
    populationT = InitialPopulationMatrix.populationT;
    ...
    PARmax = PARmax(groupparall)';
    PARmin = PARmin(groupparall)';
    PARop = PAR(groupparall);
    ...
    ind_delete = [];
    for i = 1:numel(ind)
        i_indori = groupparall(ind(i));
        if ismember(i_indori , limind) 
            [~, pos] = ismember(i_indori, limind);
            if ub(i_indori) >= limind_lb(pos)||lb(i_indori) <= limind_ub(pos)
    ...
    parlbub = [groupparall,lbuse,ubuse,PARmin * ratio,PARmax * 1/ratio];
    ...
    while 1 > 0
        warning('off','all')
        InitialPopulationMatrix = load(filepath_opmat);
        InitialPopulationMatrix = InitialPopulationMatrix.populationT;
        InitialPopulationMatrix = InitialPopulationMatrix(:,groupparall);
        ...
        lbuse = lb(groupparall);
        ubuse = ub(groupparall);
