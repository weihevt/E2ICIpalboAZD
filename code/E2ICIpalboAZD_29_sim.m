function [tout,simout,xout,mse_use] = E2ICIpalboAZD_29_sim(x0,tspan,treat,args)
...
simout = nan;
changemedia = E2ICIpalboAZD_17_changemedia(tspan);
...
if ~isempty(changemedia)
    for i = 1 : numel(changemedia)
        ...
        [t,x] = args.odefun(@(t,X)E2ICIpalboAZD_13_model(t,X,treat,args.PAR),tspan_i,x0,option);
        [warnMsg,~] = lastwarn;
    ...
    option = odeset('Events',@(t,x)child_myeventfunction(t,x,ta),...
                    'RelTol',args.RelTolval,...
                    'AbsTol',args.AbsTolval);
    [tout,xout] = args.odefun(@(t,X)E2ICIpalboAZD_13_model(t,X,treat,args.PAR),tspan_i,x0,option);
    [warnMsg,~] = lastwarn;
...
simout = E2ICIpalboAZD_14_assignval(xout,args);
tout = tout+tstart;
end
